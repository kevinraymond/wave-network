...
def test_global_semantics():
    ...
    assert torch.allclose(g[0, 0, 0], manual_calc, rtol=1e-5), \
...
def test_phase_components():
    ...
    assert (sqrt_term >= 0).all() and (sqrt_term <= 1).all(), \
        f"sqrt_term out of bounds: min={sqrt_term.min()}, max={sqrt_term.max()}"
    ...
    print(f"✓ Unit circle constraint satisfied (sqrt_term² + ratio² ≈ 1)")
...
def test_complex_representation():
    ...
    g_broadcasted = g.expand(-1, seq_len, -1)
...
def test_forward_pass():
    ...
    for mode in ["modulation", "interference"]:
        ...
        model = WaveNetwork(vocab_size=vocab_size, embedding_dim=768,
...
def test_numerical_stability():
    ...
    test_cases = [
        ...
        ("Mixed scales", torch.cat([torch.randn(2, 3, 4) * 1e-6,
                                     torch.randn(2, 3, 4) * 1e6], dim=2)),
    ]
...
def test_parameter_count():
    ...
    model = WaveNetwork(vocab_size=vocab_size, embedding_dim=embedding_dim,
    ...
    if expected_range[0] <= total_params <= expected_range[1]:
        ...
        print(f"⚠ Parameter count outside expected range: {expected_range[0]:,} - {expected_range[1]:,}")
...
def test_mode_comparison():
    ...
    model_mod = WaveNetwork(vocab_size=vocab_size, embedding_dim=768,
                           num_classes=4, mode="modulation")
    model_int = WaveNetwork(vocab_size=vocab_size, embedding_dim=768,
                           num_classes=4, mode="interference")
    ...
    assert not torch.allclose(output_mod, output_int), \
